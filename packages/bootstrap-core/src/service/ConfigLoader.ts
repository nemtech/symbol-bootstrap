...
import { Assembly, KeyName, PrivateKeyResolver } from './ConfigService';
...
export class ConfigLoader {
    ...
    public async generateRandomConfiguration(
        ...
        if (shouldCreateNemesis) {
            ...
            presetData.nemesis.nemesisSignerPrivateKey = await privateKeyResolver.resolvePrivateKey(
    ...
    public generateAddresses(
        ...
        if (typeof accounts == 'number') {
            return ConfigLoader.getArray(accounts).map(() => ConfigLoader.toConfig(Account.generateNewAccount(networkType)));
        } else {
            return accounts.map((key) => ConfigLoader.toConfig(PublicAccount.createFromPublicKey(key, networkType)));
        }
    }
    ...
    public static toConfig(account: PublicAccount | Account): ConfigAccount {
    ...
    public generateAccount(
        ...
        if (oldAccount && !newAccount) {
            ...
            return ConfigLoader.toConfig(oldAccount);
        }
        if (!oldAccount && newAccount) {
            logger.info(`${getAccountLog(newAccount)} has been provided`);
            return ConfigLoader.toConfig(newAccount);
        }
        if (oldAccount && newAccount) {
            if (oldAccount.address.equals(newAccount.address)) {
                logger.info(`Reusing ${getAccountLog(newAccount)}`);
                return { ...ConfigLoader.toConfig(oldAccount), ...ConfigLoader.toConfig(newAccount) };
            }
