...
import { Account, Address, Convert, NetworkType } from 'symbol-sdk';
...
const bootstrapCustomPreset: CustomPreset = {
    ...
    nemesis: {
        ...
        mosaics: [
            {
                accounts: [toKey('A1'), toKey('A2'), toKey('A3')],
            },
            {
                accounts: [toKey('B1'), toKey('B2'), toKey('B3')],
            },
        ],
    },
...
describe('BootstrapService config', () => {
    ...
    async function basicConfigTest(preset: string, assembly: string, constPresetObject: CustomPreset, targetFolderName: string) {
        ...
        await runConfig('upgrade no params', () =>
            service.config({ report: true, offline: true, target: target, user: BootstrapUtils.CURRENT_USER, upgrade: true, reset: false }),
        );
    }
